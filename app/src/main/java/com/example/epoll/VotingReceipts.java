            ...
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                if (task.isSuccessful()){
                    for (QueryDocumentSnapshot document : task.getResult()) {
                      //  myLists.add(new VotingReceiptsList(null,null,null));
                        myLists.add(new VotingReceiptsList(document.getId(),document.get("election_name").toString(),document.getDate("date_voted")));
                      //  Toast.makeText(VotingReceipts.this, "Election ID: " +document.getId() + "Election name :" + document.get("election_name") + " Date voted: " + document.get("date_voted"), Toast.LENGTH_LONG).show();
